...
export default function VocaCoursesPage() {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const response = await fetch('/api/public/courses?category=voca');
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      {/* 히어로 섹션 */}
      <section className="pt-36 pb-16 px-4 bg-gradient-to-b from-rose-50 to-white">
        <div className="max-w-4xl mx-auto text-center">
          <p className="text-[#86868b] text-lg mb-4">Vocabulary Course</p>
          <h1 className="text-4xl md:text-6xl font-black text-[#1d1d1f] tracking-tight">
            <span className="bg-gradient-to-r from-rose-500 via-pink-500 to-orange-400 bg-clip-text text-transparent">올톡보카</span>
          </h1>
        ...
        <div className="max-w-6xl mx-auto">
          {isLoading ? (
            <div className="flex items-center justify-center py-20">
              <svg className="animate-spin h-10 w-10 text-rose-500" viewBox="0 0 24 24">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-24">
              {courses.map((course) => (
                ...
                >
                  ...
                  {/* 정보 */}
                  <div className="p-5">
                    <h3 className="text-xl font-bold text-[#1d1d1f] mb-4 group-hover:text-rose-600 transition-colors line-clamp-2">
                      {course.title}
                    </h3>
