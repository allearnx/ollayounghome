...
export default function ReadingCoursesPage() {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const response = await fetch('/api/public/courses?category=reading');
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      {/* 히어로 섹션 */}
      <section className="pt-36 pb-16 px-4 bg-gradient-to-b from-amber-50 to-white">
        <div className="max-w-4xl mx-auto text-center">
          <p className="text-[#86868b] text-lg mb-4">Reading Course</p>
          <h1 className="text-4xl md:text-6xl font-black text-[#1d1d1f] tracking-tight">
            <span className="bg-gradient-to-r from-amber-600 via-orange-500 to-yellow-400 bg-clip-text text-transparent">리딩</span> 강의
          </h1>
          <p className="text-xl text-[#86868b] mt-6">
            독해력과 읽기 능력을 한 단계 높여주는 리딩 수업
          </p>
        </div>
      </section>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-24">
              {courses.map((course) => (
                ...
                >
                  ...
                  {/* 정보 */}
                  <div className="p-5">
                    <h3 className="text-xl font-bold text-[#1d1d1f] mb-4 group-hover:text-amber-600 transition-colors line-clamp-2">
                      {course.title}
                    </h3>
