...
export default function GrammarCoursesPage() {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const response = await fetch('/api/public/courses?category=grammar');
    ...
    <main className="min-h-screen bg-white">
      ...
      <section className="pt-36 pb-16 px-4 bg-gradient-to-b from-violet-50 to-white">
        <div className="max-w-4xl mx-auto text-center">
          <p className="text-[#86868b] text-lg mb-4">Grammar Course</p>
          <h1 className="text-4xl md:text-6xl font-black text-[#1d1d1f] tracking-tight">
            <span className="bg-gradient-to-r from-violet-600 via-purple-500 to-cyan-400 bg-clip-text text-transparent">문법</span> 강의
          </h1>
          <p className="text-xl text-[#86868b] mt-6">
            체계적인 문법 학습으로 영어의 기초를 완성하세요
          </p>
        </div>
      </section>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-24">
              {courses.map((course) => (
                ...
                >
                  ...
                  {/* 정보 */}
                  <div className="p-5">
                    <h3 className="text-xl font-bold text-[#1d1d1f] mb-4 group-hover:text-violet-600 transition-colors line-clamp-2">
                      {course.title}
                    </h3>
